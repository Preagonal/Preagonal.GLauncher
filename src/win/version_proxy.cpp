...
static const char* ogHostNames[4];
...
bool ShowServerSelectDialog(const std::string& licenseFile) {
    ...
    for (size_t i = 0; i < g_servers.size(); i++) {
        ...
        HWND hBtn = CreateWindowExA(0, "BUTTON", btnText.c_str(), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON | BS_TEXT, 10, 40 + (i * 40), 380, 30, hwnd, (HMENU)(1000 + i), GetModuleHandle(NULL), NULL);
...
extern "C" {
__declspec(dllexport) BOOL WINAPI GetFileVersionInfoA_proxy(LPCSTR a, DWORD b, DWORD c, LPVOID d) {
    if (!g_hRealVersion) return FALSE;
    auto pfn = (PFN_GetFileVersionInfoA)GetProcAddress(g_hRealVersion, "GetFileVersionInfoA");
    return pfn ? pfn(a, b, c, d) : FALSE;
}
__declspec(dllexport) BOOL WINAPI GetFileVersionInfoW_proxy(LPCWSTR a, DWORD b, DWORD c, LPVOID d) {
    if (!g_hRealVersion) return FALSE;
    auto pfn = (PFN_GetFileVersionInfoW)GetProcAddress(g_hRealVersion, "GetFileVersionInfoW");
    return pfn ? pfn(a, b, c, d) : FALSE;
}
__declspec(dllexport) DWORD WINAPI GetFileVersionInfoSizeA_proxy(LPCSTR a, LPDWORD b) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_GetFileVersionInfoSizeA)GetProcAddress(g_hRealVersion, "GetFileVersionInfoSizeA");
    return pfn ? pfn(a, b) : 0;
}
__declspec(dllexport) DWORD WINAPI GetFileVersionInfoSizeW_proxy(LPCWSTR a, LPDWORD b) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_GetFileVersionInfoSizeW)GetProcAddress(g_hRealVersion, "GetFileVersionInfoSizeW");
    return pfn ? pfn(a, b) : 0;
}
__declspec(dllexport) DWORD WINAPI VerFindFileA_proxy(DWORD a, LPCSTR b, LPCSTR c, LPCSTR d, LPSTR e, PUINT f, LPSTR g, PUINT h) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_VerFindFileA)GetProcAddress(g_hRealVersion, "VerFindFileA");
    return pfn ? pfn(a, b, c, d, e, f, g, h) : 0;
}
__declspec(dllexport) DWORD WINAPI VerFindFileW_proxy(DWORD a, LPCWSTR b, LPCWSTR c, LPCWSTR d, LPWSTR e, PUINT f, LPWSTR g, PUINT h) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_VerFindFileW)GetProcAddress(g_hRealVersion, "VerFindFileW");
    return pfn ? pfn(a, b, c, d, e, f, g, h) : 0;
}
__declspec(dllexport) DWORD WINAPI VerInstallFileA_proxy(DWORD a, LPCSTR b, LPCSTR c, LPCSTR d, LPCSTR e, LPCSTR f, LPSTR g, PUINT h) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_VerInstallFileA)GetProcAddress(g_hRealVersion, "VerInstallFileA");
    return pfn ? pfn(a, b, c, d, e, f, g, h) : 0;
}
__declspec(dllexport) DWORD WINAPI VerInstallFileW_proxy(DWORD a, LPCWSTR b, LPCWSTR c, LPCWSTR d, LPCWSTR e, LPCWSTR f, LPWSTR g, PUINT h) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_VerInstallFileW)GetProcAddress(g_hRealVersion, "VerInstallFileW");
    return pfn ? pfn(a, b, c, d, e, f, g, h) : 0;
}
__declspec(dllexport) DWORD WINAPI VerLanguageNameA_proxy(DWORD a, LPSTR b, DWORD c) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_VerLanguageNameA)GetProcAddress(g_hRealVersion, "VerLanguageNameA");
    return pfn ? pfn(a, b, c) : 0;
}
__declspec(dllexport) DWORD WINAPI VerLanguageNameW_proxy(DWORD a, LPWSTR b, DWORD c) {
    if (!g_hRealVersion) return 0;
    auto pfn = (PFN_VerLanguageNameW)GetProcAddress(g_hRealVersion, "VerLanguageNameW");
    return pfn ? pfn(a, b, c) : 0;
}
__declspec(dllexport) BOOL WINAPI VerQueryValueA_proxy(LPCVOID a, LPCSTR b, LPVOID* c, PUINT d) {
    if (!g_hRealVersion) return FALSE;
    auto pfn = (PFN_VerQueryValueA)GetProcAddress(g_hRealVersion, "VerQueryValueA");
    return pfn ? pfn(a, b, c, d) : FALSE;
}
__declspec(dllexport) BOOL WINAPI VerQueryValueW_proxy(LPCVOID a, LPCWSTR b, LPVOID* c, PUINT d) {
    if (!g_hRealVersion) return FALSE;
    auto pfn = (PFN_VerQueryValueW)GetProcAddress(g_hRealVersion, "VerQueryValueW");
    return pfn ? pfn(a, b, c, d) : FALSE;
}
}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH: {
            ...
            bool isGraal = exeName && _stricmp(exeName + 1, "Graal.exe") == 0;
            if (!isGraal) {
                return TRUE;
            }
            ...
            g_hRealVersion = LoadLibraryA(systemPath);
            ...
            if (!loaded) {
                std::string defaultPort = "14900";
                char exePath[MAX_PATH];
                if (GetModuleFileNameA(NULL, exePath, MAX_PATH)) {
                    DWORD dwHandle;
                    DWORD dwSize = GetFileVersionInfoSizeA(exePath, &dwHandle);
                    if (dwSize > 0) {
                        std::vector<char> versionInfo(dwSize);
                        if (GetFileVersionInfoA(exePath, dwHandle, dwSize, versionInfo.data())) {
                            VS_FIXEDFILEINFO* pFileInfo;
                            UINT uLen;
                            if (VerQueryValueA(versionInfo.data(), "\\", (LPVOID*)&pFileInfo, &uLen)) {
                                ...
                                if (major == 6 && minor == 113) {
                                    defaultPort = "14900";
                                } else if (major == 6 && minor == 0 && build == 3) {
                ...
                g_servers.clear();
                g_servers.push_back({"listserver.graal.in", defaultPort});
                g_servers.push_back({"listserver.graal.in", defaultPort == "14900" ? "14911" : "14900"});
                g_selectedServer = 0;
